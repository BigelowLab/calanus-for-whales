      ...
      if (paste0("abund_vs_pred_", year, "_", month, ".csv") %in% list.files(file.path(fp_out, species, version, "GAMs", "Projections")) &
        ...
        if (year == 2000 & month == 1) {
          ...
          if (paste0("abund_vs_pred_", year, "_", month, ".csv") %in% list.files(file.path(fp_out, species, version, "Biomod", "Projections"))) {
            biomod_abunds <- readr::read_csv(file.path(fp_out, species, version, "Biomod", "Projections", "abund_vs_pred_2000_1.csv"))
            biomod_full_data <- data.frame("year" = year, "month" = month, "abund" = gam_abunds$abund, "pred" = gam_abunds$pred)
          }
        } else {
          ...
          if (paste0("abund_vs_pred_", year, "_", month, ".csv") %in% list.files(file.path(fp_out, species, version, "Biomod", "Projections"))) {
            biomod_abunds <- readr::read_csv(file.path(fp_out, species, version, "Biomod", "Projections", paste0("abund_vs_pred_", year, "_", month, ".csv")))
            
            biomod_temp <- data.frame("year" = year, "month" = month, "abund" = gam_abunds$abund, "pred" = gam_abunds$pred)
            biomod_full_data <- rbind(gam_full_data, gam_temp)
          }
        }
      }
    }
