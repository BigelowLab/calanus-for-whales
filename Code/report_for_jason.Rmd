...
Model ``r params$version`` was created using ``r params$env_covars`` for ``r params$species``.  The models were produced using the ``r params$biomod_dataset`` dataset(s).  The models were used to project the probability of the study area of having a ``r params$species`` abundance of over ``r params$threshold`` per $m^2$, which is the right whale feeding threshold selected for this model and is assumed to indicate the formation of a patch sufficient for right whale feeding.  The models were then built using the species distribution modeling package, Biomod2, which builds presence-absence models using any of 10 different algorithms.  The algorithms selected were generalized additive models (GAMs), good explanatory models; boosted regression trees (BRTs), good predictive models; and random forests (RFs), highly accurate predictive models.  One model was built for each month, and then projected back onto the environmental data from that month for every year between 2000 and 2017.
...
grid.arrange(rasterGrob(ensemble1), rasterGrob(ensemble2), rasterGrob(ensemble3),
             ...
             heights = c(250, 250, 250, 250,
                                 250, 250, 250, 250, 
                                 250, 250, 250, 250),
             widths = c(250, 250, 250, 250,
                                 250, 250, 250, 250, 
                                 250, 250, 250, 250))
...
For each model, the logged actual abundance of ``r params$species`` was plotted against the predicted probability of patch formation  This was done for both the training data (``r params$biomod_dataset``) and independent data (CPR). Error bars indicate the variance.  The plots are color coded by region, either Mid-Atlantic Bight (MAB), George's Bank (GBK), or the Gulf of Maine (GOM).
...
**Figure 17.** Actual logged abundance versus predicted probability of patch formation for ``r params$species`` for a.) all 12 months and b.) all years.
...
**Figure 18.** Actual logged abundance versus predicted probability of patch formation for ``r params$species`` for a.) all 12 months and b.) all years.
...
**Figure 19.** Actual logged abundance versus predicted probability of patch formation for ``r params$species`` for a.) all 12 months and b.) all years.
...
**Figure 20.** Actual logged abundance versus predicted probability of patch formation for ``r params$species`` for a.) all 12 months and b.) all years.
...
**Figure 21.** Actual logged abundance versus predicted probability of patch formation for ``r params$species`` for a.) all 12 months and b.) all years.
...
**Figure 22.** Actual logged abundance versus predicted probability of patch formation for ``r params$species`` for a.) all 12 months and b.) all years.
...
**Figure 23.** Actual logged abundance versus predicted probability of patch formation for ``r params$species`` for a.) all 12 months and b.) all years.
...
**Figure 24.** Actual logged abundance versus predicted probability of patch formation for ``r params$species`` for a.) all 12 months and b.) all years.

## Climatological and inter-annual actual probability of patch formation vs. predicted

For each model, the actual probability of a patch of ``r params$species`` being sufficient for right whale feeding was plotted against the predicted probability of patch formation  This was done for both the training data (``r params$biomod_dataset``) and independent data (CPR). Error bars indicate the variance.  The plots are color coded by region, either Mid-Atlantic Bight (MAB), George's Bank (GBK), or the Gulf of Maine (GOM).
...
For each region, a plot was created comparing whether or not the abundance data exceeded the right whale feeding threshold (1 and 0, respectively), indicating the formation of a sufficient patch, for both the training dataset (``r params$biomod_dataset``) and an independent dataset (CPR) to the predicted probability of patch formation.  The shaded confidence interval represents variance.
